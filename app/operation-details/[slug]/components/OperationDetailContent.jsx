...
const OperationDetailContent = ({ operation }) => {
    ...
    useEffect(() => {
        ...
        // Construct new path
        const basePath = language === 'ar' ? '/operation-details' : '/en/operation-details';
    ...
    const renderContent = () => {
        if (operation.contents && Array.isArray(operation.contents)) {
            ...
            return <div dangerouslySetInnerHTML={{ __html: language === 'ar' ? (operation.content_ar || operation.content) : (operation.content_en || operation.content) }} />;
        }
    };
